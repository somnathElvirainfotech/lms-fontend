            ...
            <header className="header">
                <div className="container-fluid">
                    <div className="header-wrap">
                        <div className="logo"><Link onClick={clerSearchText} to="/courses"><img src="images/logo.png" alt="header image" /></Link></div>
                        <div className="hd-sec">
                            <div className="lt-side">
                                ...
                                {user.token && <div className="hd-src">
                                    <form onSubmit={e=>e.preventDefault()} action="#">
                                        <input
                                            ...
                                            type="text"
                                            name="search_text"
                                            placeholder={`${langObj.main_search}`} className="form-control" onKeyDown={e => eneterSearch(e)} />
                                        <button type="button" onClick={textSearch} className="btn"><i className="fa fa-search"></i></button>
                                    </form>
                                ...
                                <div className="hd-src">
                                    <div style={{ marginTop: "2px", marginLeft: "15px" }}>
                                        <select required className="form-control" onChange={languageHandler} id="language_type" name="language_type"  >
                            ...
                            {user.token && <div className="rt-side">
                                <div className="mb-hum">
                                    <div className="hamburger" id="hamburger-1" onClick={openNav}>
                                ...
                                <div id="myNav" className="menu-sec">
                                    ...
                                    <ul>
                                        ...
                                        {user.token ? <li><NavLink onClick={clerSearchText} to={"enrollments"}>Enrollments</NavLink></li> : ''}
                                        {user.token && user.user_role == 2 || user.token && user.user_role == 1 ? <li><Link onClick={clerSearchText} to="/user">User List</Link></li> : ''}

                                        {user.token && user.user_role == 2 || user.token && user.user_role == 1 ? <> <li><Link onClick={clerSearchText} to="/group">Group</Link></li> </> : ''}

                                        {user.user_role == 4 || user.user_role == 2 || user.user_role == 1 ? <li><Link onClick={clerSearchText} to="/course"> {langObj.courses} </Link></li> : ''}
                                        {user.token && user.user_role == 2 || user.token && user.user_role == 1 ? <> <li><Link onClick={clerSearchText} to="/category">Category</Link></li> </> : ''}
                                        {/** {user.token && user.user_role == 2 || user.token && user.user_role == 1 ? <> <li><a href="#">Tag</a></li> </> : ''}  */}
                                        {user.token && user.user_role == 2 || user.token && user.user_role == 1 ? <> <li><Link onClick={clerSearchText} to="/language">Language</Link></li> </> : ''}
                                        {user.token && user.user_role == 2 || user.token && user.user_role == 1 ? <> <li><Link onClick={clerSearchText} to="/qualification">Qualification</Link></li> </> : ''}
                                        {user.token && user.user_role == 2 || user.token && user.user_role == 1 || user.token && user.user_role == 4 ? <> <li><Link onClick={clerSearchText} to="/task">Task</Link></li> </> : ''}
                                ...
                                <div className="hd-loging-sign">
                                    {!user.token &&
                                        <ul>
                                            <li><a data-toggle="modal" data-target="#loginform" href="#">{langObj.login}</a></li>
                                            { /* <li><a href="/signup">Sign up</a></li> */}
                                        </ul>
                                    }
                                    {user.token &&
                                        <ul>
                                            <li><Link to="#" onClick={Logout}>{langObj.logout}</Link></li>
                                        </ul>
                                    }
                                </div>
                            </div>}
